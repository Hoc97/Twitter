    ...
    options: async (value: string, { req }) => {
      ...
      try {
        ...
        if (user.forgot_password_token !== value) {
          throw new Error(USERS_MESSAGES.INVALID_FORGOT_PASSWORD_TOKEN)
        }
...
const dateOfBirthSchema: ParamSchema = {
  errorMessage: USERS_MESSAGES.DATE_OF_BIRTH_MUST_BE_ISOSTRING,
  isISO8601: {
    options: {
      strict: true,
      ...
      email: {
        ...
        custom: {
          options: async (email, { req }) => {
            const user = await usersService.checkLogin({ email, password: req.body.password })
            if (!user) {
          ...
          options: async (value, { req }) => {
            ...
            try {
              const [decoded_refresh_token, refresh_token] = await Promise.all([
                verifyToken({ token: value, privateKey: process.env.JWT_SECRET as string }),
